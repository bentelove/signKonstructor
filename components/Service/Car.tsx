...
export default function Car({x,y,height=400,width=250,rotate=0,color="black"}:LiftProps) {
    const widthCar = 199;
    const heightCar = 400;
    const widthG = 47.032;
    return (
        <g transform={`matrix(1,0,0,1,${x},${y})`}>
            <g transform={`rotate(${rotate})`}>
                <g fill={color} transform={`matrix(${(height/widthG)*(width/widthCar)*(heightCar/height)},0,0,${height/widthG},${(-height/2)*(width/widthCar)/(height/heightCar)},${-height/2})`} height={height} width={width} viewBox="0 0 47.032 47.032">
